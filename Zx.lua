...
local LS = New("UIStroke",{Color=RGB(0),Thickness=2,Transparency=0.3,Parent=Load})
...
local LB = New("Frame",{Size=UDim2.new(0,0,0,8),Position=UDim2.new(0.5,-140,0.5,85),BackgroundColor3=RGB(1),BorderSizePixel=0,Parent=Load})
New("UICorner",{CornerRadius=UDim.new(0,4),Parent=LB})

local Notif = New("Frame",{Name="Notifs",Size=UDim2.new(0,260,0,320),Position=UDim2.new(0.5,-130,0.85,0),BackgroundTransparency=1,BorderSizePixel=0,Parent=ScreenGui})
local NList = {}
...
local Panel = New("Frame",{Name="Main",Size=UDim2.new(0,270,0,430),Position=UDim2.new(0.5,-135,0.5,1000),BackgroundColor3=Color3.fromRGB(8,8,8),BorderSizePixel=0,Parent=ScreenGui,Visible=false})
New("UICorner",{CornerRadius=UDim.new(0,14),Parent=Panel})
local PS = New("UIStroke",{Color=RGB(0),Thickness=2,Transparency=0.2,Parent=Panel})
local TB = New("Frame",{Size=UDim2.new(1,0,0,48),BackgroundColor3=Color3.fromRGB(12,12,12),Parent=Panel})
...
local function Btn(txt,y,s)
    local b = New("TextButton",{Size=UDim2.new(0.88,0,0,38),Position=UDim2.new(0.06,0,y,0),BackgroundColor3=s and Color3.fromRGB(0,80,0) or Color3.fromRGB(20,20,20),BorderSizePixel=0,Text=txt,Font=Enum.Font.GothamBold,TextColor3=Color3.new(1,1,1),TextSize=14,TextXAlignment=Enum.TextXAlignment.Left,Parent=Panel})    New("UICorner",{CornerRadius=UDim.new(0,8),Parent=b})
...
local AB = Btn("Mira: OFF",0.12,false)
local CB = Btn("RGB Círculo: OFF",0.24,false)
local EB = Btn("ESP: OFF",0.36,false)
local RB = Btn("RGB ESP: OFF",0.48,false)

local SL = New("Frame",{Size=UDim2.new(0.88,0,0,45),Position=UDim2.new(0.06,0,0.60,0),BackgroundColor3=Color3.fromRGB(16,16,16),BorderSizePixel=0,Parent=Panel})
...
local function Checkbox(txt,y,active)
    local f = New("Frame",{Size=UDim2.new(0.88,0,0,24),Position=UDim2.new(0.06,0,y,0),BackgroundColor3=Color3.fromRGB(18,18,18),BorderSizePixel=0,Parent=Panel})
    ...
    if active then
        New("TextLabel",{Size=UDim2.new(1,0,1,0),BackgroundTransparency=1,Text="✓",Font=Enum.Font.GothamBold,TextColor3=Color3.new(1,1,1),TextSize=10,TextXAlignment=Enum.TextXAlignment.Center,Parent=ch})
    end
    ...
    f.InputBegan:Connect(function(i)
        if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then
            for _,v in pairs(parts) do
                local box = v:FindFirstChildOfClass("Frame")
                box.BackgroundColor3 = Color3.fromRGB(35,35,35)
                local mark = box:FindFirstChildOfClass("TextLabel")
...
Checkbox("Cabeça",0.70,true)
Checkbox("Peito",0.75,false)
Checkbox("Pé",0.80,false)
...
Panel.InputBegan:Connect(function(i)
    if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then
        ...
        if rx<28 or rx>Panel.AbsoluteSize.X-28 then dP=true; dSt=i.Position; pSt=Panel.Position end
    end
end)
UserInputService.InputChanged:Connect(function(i)
    ...
    if aO and cam then
        ...
        for _,p in ipairs(Players:GetPlayers()) do
            if p~=LocalPlayer and p.Character then
                local pn = aimPart=="Cabeça" and "Head" or aimPart=="Peito" and "UpperTorso" or "LeftLowerLeg"
...
task.spawn(function()
    for i=0,100,2 do
        local dots = math.floor(i/20)%3
        LD.Text = "Carregando"..string.rep(".",dots+1)        LB.Size=UDim2.new(0,i*2.8,0,8); LB.Position=UDim2.new(0.5,-140+i*1.4,0.5,85); LB.BackgroundColor3=RGB(i*0.05)
        LS.Color=RGB(i*0.08); task.wait(0.018)
    end
    T(Load,{BackgroundTransparency=1,Size=UDim2.new(0,380,0,220)},0.4)
    T(LS,{Transparency=1},0.3); T(LB,{BackgroundTransparency=1},0.3); T(LD,{TextTransparency=1},0.3)
    task.wait(0.4); Load:Destroy()
    Panel.Visible=true; T(Panel,{Position=UDim2.new(0.5,-135,0.5,-215)},0.6)
    Notify("Zx Carregado")
end)
