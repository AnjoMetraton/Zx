...
local SL = New("Frame",{Size=UDim2.new(0.85,0,0,45),Position=UDim2.new(0.075,0,0.59,0),BackgroundColor3=Color3.fromRGB(16,16,16),BorderSizePixel=0,Parent=Panel,Active=true})
...
local ST = New("Frame",{Size=UDim2.new(0,12,0,12),Position=UDim2.new(0.5,-6,0.5,-6),BackgroundColor3=Color3.new(1,1,1),BorderSizePixel=0,Parent=STR,Active=true})
New("UICorner",{CornerRadius=UDim.new(0.5,0),Parent=ST})

local aimPart = "Cabeça"
local checkboxes = {}
local function Checkbox(txt,y,active)
	local f = New("Frame",{Size=UDim2.new(0.85,0,0,24),Position=UDim2.new(0.075,0,y,0),BackgroundColor3=Color3.fromRGB(18,18,18),BorderSizePixel=0,Parent=Panel,Active=true})
	...
	f.InputBegan:Connect(function(i)
		if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then
			...
			if not ch:FindFirstChildOfClass("TextLabel") then
...
AB.MouseButton1Click:Connect(function()
	aO = not aO
	AB.Text = "Mira: "..(aO and "ON" or "OFF")
	AB.BackgroundColor3 = aO and Color3.fromRGB(0,80,0) or Color3.fromRGB(20,20,20)
	Notify("Mira "..(aO and "Ativada" or "Desativada"))
	CV.Visible = aO
end)

CB.MouseButton1Click:Connect(function()
	rCO = not rCO
	CB.Text = "RGB Círculo: "..(rCO and "ON" or "OFF")
	Notify("RGB Círculo "..(rCO and "Ativado" or "Desativado"))
end)

EB.MouseButton1Click:Connect(function()
	eO = not eO
	EB.Text = "ESP: "..(eO and "ON" or "OFF")
	EB.BackgroundColor3 = eO and Color3.fromRGB(0,80,0) or Color3.fromRGB(20,20,20)
	Notify("ESP "..(eO and "Ativado" or "Desativado"))
	if not eO then
		for _,o in pairs(eC) do o:Destroy() end
		eC = {}
	end
end)
...
RB.MouseButton1Click:Connect(function()
	rEO = not rEO
	RB.Text = "RGB ESP: "..(rEO and "ON" or "OFF")
	Notify("RGB ESP "..(rEO and "Ativado" or "Desativado"))
end)
...
UserInputService.InputChanged:Connect(function(i)
	if dS and (i.UserInputType==Enum.UserInputType.MouseMovement or i.UserInputType==Enum.UserInputType.Touch) then
		...
		ST.Position = UDim2.new(x,-6,0.5,-6)
		cS = math.floor(x*900+100)
		SLT.Text = "Raio: "..cS
		CV.Size = UDim2.new(0,cS*2,0,cS*2)
		CV.Position = UDim2.new(0.5,-cS,0.5,-cS)
	end
end)
...
Panel.InputBegan:Connect(function(i)
	if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then
		...
		if rx<25 or rx>Panel.AbsoluteSize.X-25 then
			dP = true
			dSt = i.Position
			pSt = Panel.Position
		end
...
RunService.RenderStepped:Connect(function()
	...
	if aO and cam then
		...
		if tgt and tgt:IsA("BasePart") then
			...
			cam.CFrame = cam.CFrame:Lerp(CFrame.lookAt(cam.CFrame.Position,tgt.Position),0.18)
		else
	...
	local t = tick()
	PS.Color = RGB(t)
	if rCO and CV.Visible then
		CVS.Color = RGB(t)
		CVF.BackgroundColor3 = RGB(t)
	elseif CV.Visible then
		CVS.Color = Color3.new(0.6,0,1)
		CVF.BackgroundColor3 = Color3.new(0.6,0,1)
	end
end)
		...
		for _,p in ipairs(Players:GetPlayers()) do
			if p~=LocalPlayer and p.Character then
				if not eC[p] then
					...
					h.FillColor = rEO and RGB(t) or Color3.new(0.6,0,1)
					...
					if rEO then
						eC[p].FillColor = RGB(t)
					else
						eC[p].FillColor = Color3.new(0.6,0,1)
					end
				end
			end
		end
...
task.spawn(function()
	for i=0,100,2 do
		...
		LB.Size = UDim2.new(0,i*3.2,0,8)
		LB.Position = UDim2.new(0.5,-160+i*1.6,0.5,80)
		LB.BackgroundColor3 = RGB(i*0.05)
		LT.Text = i.."%"
		LS.Color = RGB(i*0.08)
		task.wait(0.018)
	end
	T(Load,{BackgroundTransparency=1,Size=UDim2.new(0,380,0,220)},0.4)
	T(LS,{Transparency=1},0.3)
	T(LB,{BackgroundTransparency=1},0.3)
	T(LD,{TextTransparency=1},0.3)
	T(LT,{TextTransparency=1},0.3)
	task.wait(0.4)
	Load:Destroy()
	Panel.Visible = true
	T(Panel,{Position=UDim2.new(0.5,-140,0.5,-230)},0.65)
	Notify("Zx Carregado")
end)
