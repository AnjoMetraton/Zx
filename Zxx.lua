...
local function ColorDistance(c1,c2)
	local dr=c1.R-c2.R; local dg=c1.G-c2.G; local db=c1.B-c2.B
...
local function GetCharacterFromWorkspace(player)
	...
	if pf then return pf:FindFirstChild(player.Name) end
	return player.Character
end

local function GetTeamColorFromNametag(char)
	if not char then return nil end
	local nt=char:FindFirstChild("Nametag"); if not nt then return nil end
	local lbl=nt:FindFirstChild("Player"); if not lbl then return nil end
	if lbl:IsA("TextLabel") then return lbl.TextColor3 end
	if lbl:IsA("ImageLabel") then return lbl.ImageColor3 end
	if lbl:IsA("Frame") then return lbl.BackgroundColor3 end
	return nil
end
...
local function IsSameTeam(playerB)
	...
	local nB=GetTeamColorFromNametag(charB); if not nB then return false end
	local teamB=ClassifyTeam(nB); if not teamB then return false end
...
local NotifHolder=New("Frame",{Size=UDim2.new(0,270,0,320),Position=UDim2.new(0.5,-135,0.80,0),BackgroundTransparency=1,BorderSizePixel=0,Parent=ScreenGui})
local NotifList={}
local function Notify(txt)
	local n=New("Frame",{Size=UDim2.new(1,0,0,44),Position=UDim2.new(0,0,1,0),BackgroundColor3=Color3.fromRGB(4,2,10),BackgroundTransparency=0,BorderSizePixel=0,Parent=NotifHolder})
	New("UICorner",{CornerRadius=UDim.new(0,10),Parent=n})
	New("UIStroke",{Color=Color3.fromRGB(110,0,230),Thickness=1.3,Parent=n})
	local accent=New("Frame",{Size=UDim2.new(0,3,0.6,0),Position=UDim2.new(0,0,0.2,0),BackgroundColor3=Color3.fromRGB(140,0,255),BorderSizePixel=0,Parent=n})
	New("UICorner",{CornerRadius=UDim.new(0,10),Parent=accent})
	New("UIGradient",{Color=ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.fromRGB(180,0,255)),ColorSequenceKeypoint.new(1,Color3.fromRGB(0,180,255))}),Rotation=90,Parent=accent})
	New("TextLabel",{Size=UDim2.new(0,28,1,0),Position=UDim2.new(0,8,0,0),BackgroundTransparency=1,Text="⬡",Font=Enum.Font.GothamBold,TextColor3=Color3.fromRGB(140,0,255),TextSize=14,TextXAlignment=Enum.TextXAlignment.Center,Parent=n})
	New("TextLabel",{Size=UDim2.new(1,-42,1,0),Position=UDim2.new(0,38,0,0),BackgroundTransparency=1,Text=txt,Font=Enum.Font.GothamBold,TextColor3=Color3.new(1,1,1),TextSize=12,TextXAlignment=Enum.TextXAlignment.Left,Parent=n})
	Tween(n,{Position=UDim2.new(0,0,1-(#NotifList+1)*0.17,0)},0.35)
	table.insert(NotifList,n)
	task.delay(3,function()
		Tween(n,{Position=UDim2.new(0,0,1.6,0),BackgroundTransparency=1},0.3)
		task.wait(0.3); n:Destroy(); table.remove(NotifList,1)
	end)
end

local Panel=New("Frame",{Size=UDim2.new(0,320,0,500),Position=UDim2.new(0.5,-160,0.5,1200),BackgroundColor3=Color3.new(0,0,0),BorderSizePixel=0,ClipsDescendants=true,Visible=false,Parent=ScreenGui})
...
local PAccent=New("Frame",{Size=UDim2.new(0.5,0,0,2),Position=UDim2.new(0.25,0,0,0),BackgroundColor3=Color3.fromRGB(140,0,255),BackgroundTransparency=0,BorderSizePixel=0,Parent=Panel})
New("UICorner",{CornerRadius=UDim.new(0,2),Parent=PAccent})
New("UIGradient",{Color=ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.fromRGB(180,0,255)),ColorSequenceKeypoint.new(0.5,Color3.fromRGB(0,180,255)),ColorSequenceKeypoint.new(1,Color3.fromRGB(180,0,255))}),Parent=PAccent})

local TopBar=New("Frame",{Size=UDim2.new(1,0,0,54),BackgroundColor3=Color3.new(0,0,0),ZIndex=3,Parent=Panel})
...
New("TextLabel",{Size=UDim2.new(0,28,1,0),Position=UDim2.new(0,14,0,0),BackgroundTransparency=1,Text="⬡",Font=Enum.Font.GothamBold,TextColor3=Color3.fromRGB(140,0,255),TextSize=18,TextXAlignment=Enum.TextXAlignment.Center,ZIndex=3,Parent=TopBar})
New("TextLabel",{Size=UDim2.new(1,-80,1,0),Position=UDim2.new(0,38,0,0),BackgroundTransparency=1,Text="ZX MENU",Font=Enum.Font.GothamBold,TextColor3=Color3.new(1,1,1),TextSize=17,TextXAlignment=Enum.TextXAlignment.Left,ZIndex=3,Parent=TopBar})
local CloseBtn=New("TextButton",{Size=UDim2.new(0,32,0,32),Position=UDim2.new(1,-42,0.5,-16),BackgroundColor3=Color3.fromRGB(18,5,35),BackgroundTransparency=0,BorderSizePixel=0,Text="✕",Font=Enum.Font.GothamBold,TextColor3=Color3.fromRGB(180,80,255),TextSize=14,ZIndex=4,Parent=TopBar,Active=true})
...
local TopDiv=New("Frame",{Size=UDim2.new(1,0,0,1),Position=UDim2.new(0,0,1,0),BackgroundColor3=Color3.fromRGB(80,0,180),BackgroundTransparency=0.3,BorderSizePixel=0,ZIndex=3,Parent=TopBar})
New("UIGradient",{Color=ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.new(0,0,0)),ColorSequenceKeypoint.new(0.2,Color3.fromRGB(140,0,255)),ColorSequenceKeypoint.new(0.8,Color3.fromRGB(0,180,255)),ColorSequenceKeypoint.new(1,Color3.new(0,0,0))}),Parent=TopDiv})

local ScrollFrame=New("ScrollingFrame",{Size=UDim2.new(1,0,1,-54),Position=UDim2.new(0,0,0,54),BackgroundTransparency=1,BorderSizePixel=0,ScrollBarThickness=2,ScrollBarImageColor3=Color3.fromRGB(120,0,240),ScrollBarImageTransparency=0.3,CanvasSize=UDim2.new(0,0,0,1050),ScrollingDirection=Enum.ScrollingDirection.Y,ElasticBehavior=Enum.ElasticBehavior.Never,Parent=Panel})
...
local function SetBtn(btn,state)
	...
	local textLbl=btn:FindFirstChild("TextLbl")
	local iconLbl=btn:FindFirstChild("IconLbl")
	if textLbl then textLbl.TextColor3=state and Color3.fromRGB(200,140,255) or Color3.fromRGB(160,155,175) end
	if iconLbl then iconLbl.TextColor3=state and Color3.fromRGB(160,0,255) or Color3.fromRGB(70,50,100) end
...
local function UpdateBtnText(btn,txt)
	local textLbl=btn:FindFirstChild("TextLbl")
	if textLbl then textLbl.Text=txt end
end

local BtnAim=MakeBtn("MIRA: OFF",10,false)
local BtnAimFix=MakeBtn("AIMBOT FIX: OFF",62,false)
local BtnCircle=MakeBtn("RGB CÍRCULO: OFF",114,false)
local BtnESP=MakeBtn("ESP: OFF",166,false)
local BtnRGBESP=MakeBtn("RGB ESP: OFF",218,false)
local BtnRGBName=MakeBtn("ESP NOME RGB: OFF",270,false)
local BtnKeybind=MakeBtn("⌨  ALTERAR TECLAS",322,false)

local SliderOuter=New("Frame",{Size=UDim2.new(0.88,0,0,44),Position=UDim2.new(0.06,0,0,380),BackgroundColor3=Color3.fromRGB(5,3,12),BorderSizePixel=0,Parent=ScrollFrame,Active=true})
New("UICorner",{CornerRadius=UDim.new(0,10),Parent=SliderOuter})
New("UIStroke",{Color=Color3.fromRGB(30,20,50),Thickness=1,Parent=SliderOuter})
local SliderLabel=New("TextLabel",{Size=UDim2.new(1,-10,0,16),Position=UDim2.new(0,10,0,5),BackgroundTransparency=1,Text="RAIO: 30",Font=Enum.Font.GothamBold,TextColor3=Color3.fromRGB(120,80,200),TextSize=11,TextXAlignment=Enum.TextXAlignment.Left,Parent=SliderOuter})
local SliderTrack=New("Frame",{Size=UDim2.new(0.86,0,0,4),Position=UDim2.new(0.07,0,0,28),BackgroundColor3=Color3.fromRGB(10,7,18),BorderSizePixel=0,Parent=SliderOuter})
...
local function SectionLabel(txt,y)
	local f=New("Frame",{Size=UDim2.new(0.88,0,0,20),Position=UDim2.new(0.06,0,0,y),BackgroundTransparency=1,Parent=ScrollFrame})
	New("Frame",{Size=UDim2.new(0.3,0,0,1),Position=UDim2.new(0,0,0.5,0),BackgroundColor3=Color3.fromRGB(50,30,80),BackgroundTransparency=0.3,BorderSizePixel=0,Parent=f})
	New("Frame",{Size=UDim2.new(0.3,0,0,1),Position=UDim2.new(0.7,0,0.5,0),BackgroundColor3=Color3.fromRGB(50,30,80),BackgroundTransparency=0.3,BorderSizePixel=0,Parent=f})
	New("TextLabel",{Size=UDim2.new(0.4,0,1,0),Position=UDim2.new(0.3,0,0,0),BackgroundTransparency=1,Text=txt,Font=Enum.Font.GothamBold,TextColor3=Color3.fromRGB(80,55,120),TextSize=10,TextXAlignment=Enum.TextXAlignment.Center,Parent=f})
end

SectionLabel("PARTE DO ALVO",440)

local aimPart="Cabeça"
local checkboxes={}
local function MakeCB(label,y,active)
	local f=New("Frame",{Size=UDim2.new(0.88,0,0,26),Position=UDim2.new(0.06,0,0,y),BackgroundColor3=Color3.fromRGB(5,3,12),BorderSizePixel=0,Parent=ScrollFrame,Active=true})
	New("UICorner",{CornerRadius=UDim.new(0,8),Parent=f})
	New("UIStroke",{Color=Color3.fromRGB(28,16,50),Thickness=1,Parent=f})
	local accent=New("Frame",{Size=UDim2.new(0,3,0.6,0),Position=UDim2.new(0,0,0.2,0),BackgroundColor3=Color3.fromRGB(100,0,220),BorderSizePixel=0,Parent=f})
	New("UICorner",{CornerRadius=UDim.new(0,2),Parent=accent})
	local ind=New("Frame",{Size=UDim2.new(0,13,0,13),Position=UDim2.new(0,12,0.5,-6),BackgroundColor3=active and Color3.fromRGB(80,0,180) or Color3.fromRGB(15,10,28),BorderSizePixel=0,Parent=f})
	New("UICorner",{CornerRadius=UDim.new(0,3),Parent=ind})
	...
	New("TextLabel",{Size=UDim2.new(1,-40,1,0),Position=UDim2.new(0,32,0,0),BackgroundTransparency=1,Text=label,Font=Enum.Font.GothamBold,TextColor3=Color3.fromRGB(170,150,210),TextSize=12,TextXAlignment=Enum.TextXAlignment.Left,Parent=f})
...
MakeCB("Cabeça",470,true); MakeCB("Peito",504,false); MakeCB("Pé",538,false)

SectionLabel("HITBOX",576)

local HBox=New("Frame",{Size=UDim2.new(0.88,0,0,86),Position=UDim2.new(0.06,0,0,604),BackgroundColor3=Color3.fromRGB(5,3,12),BorderSizePixel=0,Parent=ScrollFrame,Active=true})
New("UICorner",{CornerRadius=UDim.new(0,10),Parent=HBox})
New("UIStroke",{Color=Color3.fromRGB(28,16,50),Thickness=1,Parent=HBox})
local HBoxToggle=New("TextButton",{Size=UDim2.new(0,38,0,22),Position=UDim2.new(1,-46,0,8),BackgroundColor3=Color3.fromRGB(15,10,28),BorderSizePixel=0,Text="",Parent=HBox,Active=true})
New("UICorner",{CornerRadius=UDim.new(0,11),Parent=HBoxToggle})
New("UIStroke",{Color=Color3.fromRGB(70,0,150),Thickness=1,Parent=HBoxToggle})
local HBoxDot=New("Frame",{Size=UDim2.new(0,15,0,15),Position=UDim2.new(0,3,0.5,-7),BackgroundColor3=Color3.fromRGB(60,40,90),BorderSizePixel=0,Parent=HBoxToggle})
New("UICorner",{CornerRadius=UDim.new(0.5,0),Parent=HBoxDot})
local HBoxLabel=New("TextLabel",{Size=UDim2.new(1,-54,0,18),Position=UDim2.new(0,10,0,6),BackgroundTransparency=1,Text="HITBOX: 1x",Font=Enum.Font.GothamBold,TextColor3=Color3.fromRGB(130,90,210),TextSize=11,TextXAlignment=Enum.TextXAlignment.Left,Parent=HBox})
local HBoxTrack=New("Frame",{Size=UDim2.new(0.78,0,0,4),Position=UDim2.new(0.06,0,0,38),BackgroundColor3=Color3.fromRGB(10,7,18),BorderSizePixel=0,Parent=HBox})
...
New("Frame",{Size=UDim2.new(0.9,0,0,1),Position=UDim2.new(0.05,0,0,56),BackgroundColor3=Color3.fromRGB(25,15,45),BackgroundTransparency=0.2,BorderSizePixel=0,Parent=HBox})
New("TextLabel",{Size=UDim2.new(0.6,0,0,24),Position=UDim2.new(0,10,0,60),BackgroundTransparency=1,Text="RGB BORDA",Font=Enum.Font.GothamBold,TextColor3=Color3.fromRGB(130,90,210),TextSize=11,TextXAlignment=Enum.TextXAlignment.Left,Parent=HBox})
local HBoxRGBToggle=New("TextButton",{Size=UDim2.new(0,38,0,22),Position=UDim2.new(1,-46,0,62),BackgroundColor3=Color3.fromRGB(15,10,28),BorderSizePixel=0,Text="",Parent=HBox,Active=true})
New("UICorner",{CornerRadius=UDim.new(0,11),Parent=HBoxRGBToggle})
New("UIStroke",{Color=Color3.fromRGB(70,0,150),Thickness=1,Parent=HBoxRGBToggle})
local HBoxRGBDot=New("Frame",{Size=UDim2.new(0,15,0,15),Position=UDim2.new(0,3,0.5,-7),BackgroundColor3=Color3.fromRGB(60,40,90),BorderSizePixel=0,Parent=HBoxRGBToggle})
New("UICorner",{CornerRadius=UDim.new(0.5,0),Parent=HBoxRGBDot})

SectionLabel("OPÇÕES AIMBOT",706)

local function MakeRow(lbl,y)
	local row=New("Frame",{Size=UDim2.new(0.88,0,0,38),Position=UDim2.new(0.06,0,0,y),BackgroundColor3=Color3.fromRGB(5,3,12),BorderSizePixel=0,Parent=ScrollFrame,Active=true})
	New("UICorner",{CornerRadius=UDim.new(0,10),Parent=row})
	New("UIStroke",{Color=Color3.fromRGB(28,16,50),Thickness=1,Parent=row})
	local accent2=New("Frame",{Size=UDim2.new(0,3,0.6,0),Position=UDim2.new(0,0,0.2,0),BackgroundColor3=Color3.fromRGB(100,0,220),BorderSizePixel=0,Parent=row})
	New("UICorner",{CornerRadius=UDim.new(0,2),Parent=accent2})
	New("TextLabel",{Size=UDim2.new(1,-58,1,0),Position=UDim2.new(0,12,0,0),BackgroundTransparency=1,Text=lbl,Font=Enum.Font.GothamBold,TextColor3=Color3.fromRGB(170,150,210),TextSize=11,TextXAlignment=Enum.TextXAlignment.Left,Parent=row})
	local tog=New("TextButton",{Size=UDim2.new(0,38,0,22),Position=UDim2.new(1,-46,0.5,-11),BackgroundColor3=Color3.fromRGB(15,10,28),BorderSizePixel=0,Text="",Parent=row,Active=true})
	New("UICorner",{CornerRadius=UDim.new(0,11),Parent=tog})
	New("UIStroke",{Color=Color3.fromRGB(70,0,150),Thickness=1,Parent=tog})
	local dot=New("Frame",{Size=UDim2.new(0,15,0,15),Position=UDim2.new(0,3,0.5,-7),BackgroundColor3=Color3.fromRGB(60,40,90),BorderSizePixel=0,Parent=tog})
	New("UICorner",{CornerRadius=UDim.new(0.5,0),Parent=dot})
	return tog,dot
end
local TeamToggle,TeamDot=MakeRow("IGNORAR PRÓPRIO TIME",732)
local WallToggle,WallDot=MakeRow("IGNORAR ATRAVÉS DA PAREDE",778)

SectionLabel("MEU TIME",828)

local TeamBtnRed=New("TextButton",{Size=UDim2.new(0.42,0,0,38),Position=UDim2.new(0.06,0,0,856),BackgroundColor3=Color3.fromRGB(35,5,5),BorderSizePixel=0,Text="● VERMELHO",Font=Enum.Font.GothamBold,TextColor3=Color3.fromRGB(229,72,72),TextSize=12,TextXAlignment=Enum.TextXAlignment.Center,Parent=ScrollFrame,Active=true})
New("UICorner",{CornerRadius=UDim.new(0,10),Parent=TeamBtnRed})
local TeamBtnRedStroke=New("UIStroke",{Color=Color3.fromRGB(90,18,18),Thickness=1.5,Parent=TeamBtnRed})

local TeamBtnBlue=New("TextButton",{Size=UDim2.new(0.42,0,0,38),Position=UDim2.new(0.52,0,0,856),BackgroundColor3=Color3.fromRGB(5,12,35),BorderSizePixel=0,Text="● AZUL",Font=Enum.Font.GothamBold,TextColor3=Color3.fromRGB(72,171,229),TextSize=12,TextXAlignment=Enum.TextXAlignment.Center,Parent=ScrollFrame,Active=true})
New("UICorner",{CornerRadius=UDim.new(0,10),Parent=TeamBtnBlue})
local TeamBtnBlueStroke=New("UIStroke",{Color=Color3.fromRGB(18,55,90),Thickness=1.5,Parent=TeamBtnBlue})
...
local PopUp=New("TextButton",{Size=UDim2.new(0,58,0,30),Position=UDim2.new(1,-70,0,60),BackgroundColor3=Color3.fromRGB(4,2,10),BorderSizePixel=0,Text="⬡ ZX",Font=Enum.Font.GothamBold,TextColor3=Color3.fromRGB(160,80,255),TextSize=12,Visible=false,Parent=ScreenGui,Active=true})
New("UICorner",{CornerRadius=UDim.new(0,10),Parent=PopUp})
New("UIStroke",{Color=Color3.fromRGB(100,0,220),Thickness=1.3,Parent=PopUp})
...
local function SetToggle(tog,dot,state)
	...
	Tween(dot,{Position=state and UDim2.new(1,-18,0.5,-7) or UDim2.new(0,3,0.5,-7)},0.2)
...
local function getTargetPart(player)
	...
	local char=GetCharacterFromWorkspace(player); if not char then return nil end
...
local function getClosest(cam)
	...
	for _,player in ipairs(Players:GetPlayers()) do
		...
		local char=GetCharacterFromWorkspace(player); if not char then continue end
		local hum=char:FindFirstChildOfClass("Humanoid")
		local root=char:FindFirstChild("HumanoidRootPart")
		if not(hum and hum.Health>0 and root) then continue end
		if ignoreTeam and IsSameTeam(player) then continue end
...
local function createESPName(p)
	local char=GetCharacterFromWorkspace(p); if not char then return end
	local root=char:FindFirstChild("HumanoidRootPart"); if not root then return end
	removeESPName(p)
	local bb=Instance.new("BillboardGui")
	bb.Size=UDim2.new(0,130,0,28); bb.StudsOffset=Vector3.new(0,3.5,0); bb.AlwaysOnTop=true; bb.ResetOnSpawn=false; bb.Adornee=root; bb.Parent=root
...
local function restoreHitboxes()
	for p,sz in pairs(hitboxOriginals) do
		if p~=LocalPlayer then
			...
			if char then local r=char:FindFirstChild("HumanoidRootPart"); if r and sz then r.Size=sz end end
		end
		removeHBVisual(p)
	end
	hitboxOriginals={}
	for _,sb in pairs(hitboxVisualCache) do if sb then sb:Destroy() end end
	hitboxVisualCache={}
end
...
BtnRGBName.MouseButton1Click:Connect(function()
	rgbNameOn=not rgbNameOn; UpdateBtnText(BtnRGBName,"ESP NOME RGB: "..(rgbNameOn and "ON" or "OFF")); SetBtn(BtnRGBName,rgbNameOn)
...
TeamBtnRed.MouseButton1Click:Connect(function()
	mySelectedTeam="red"
	TeamBtnRedStroke.Color=Color3.fromRGB(229,72,72); TeamBtnRedStroke.Thickness=2.5
	TeamBtnBlueStroke.Color=Color3.fromRGB(18,55,90); TeamBtnBlueStroke.Thickness=1.5
	TeamBtnRed.BackgroundColor3=Color3.fromRGB(55,8,8); TeamBtnBlue.BackgroundColor3=Color3.fromRGB(5,12,35)
	Notify("TIME: VERMELHO SELECIONADO")
end)

TeamBtnBlue.MouseButton1Click:Connect(function()
	mySelectedTeam="blue"
	TeamBtnBlueStroke.Color=Color3.fromRGB(72,171,229); TeamBtnBlueStroke.Thickness=2.5
	TeamBtnRedStroke.Color=Color3.fromRGB(90,18,18); TeamBtnRedStroke.Thickness=1.5
...
local function CloseMenu()
	menuOpen=false; Tween(Panel,{Position=UDim2.new(0.5,-160,0.5,1200)},0.4)
	task.wait(0.35); Panel.Visible=false
	PopUp.Size=UDim2.new(0,0,0,30); PopUp.Visible=true; Tween(PopUp,{Size=UDim2.new(0,58,0,30)},0.35)
end
local function OpenMenu()
	menuOpen=true; Tween(PopUp,{Size=UDim2.new(0,0,0,30)},0.25); task.wait(0.22)
	PopUp.Visible=false; Panel.Visible=true; Tween(Panel,{Position=UDim2.new(0.5,-160,0.5,-250)},0.45)
end
...
RunService:BindToRenderStep("ZxAimFix",Enum.RenderPriority.Camera.Value+1,function()
	...
	local needsNew=not lockedPlayer or not GetCharacterFromWorkspace(lockedPlayer)
	if not needsNew then
		local hum=GetCharacterFromWorkspace(lockedPlayer):FindFirstChildOfClass("Humanoid")
		needsNew=not hum or hum.Health<=0
	end
...
RunService.RenderStepped:Connect(function()
	...
	if aimOn and not aimFixOn then
		...
		if cam then
			...
			if closest then
				...
				if part and part.Parent then
					local sp,on=cam:WorldToViewportPoint(part.Position)
					if on then Crosshair.Position=UDim2.new(0,sp.X-11,0,sp.Y-11); cam.CFrame=cam.CFrame:Lerp(CFrame.new(cam.CFrame.Position,part.Position),0.2) end
				end
	...
	if LockFrame.Visible then
		...
		for _,c in ipairs(LockFrame:GetChildren()) do
			if c:IsA("Frame") and c~=LockDot then c.BackgroundTransparency=pulse end
		end
	end
...
RunService.Heartbeat:Connect(function()
	if hitboxOn then
		local t=tick(); local bc=rgbHitboxOn and RGB(t) or Color3.fromRGB(120,0,255); local ns=math.max(hitboxSize*4,1)
		for _,p in ipairs(Players:GetPlayers()) do
			...
			if char then
				...
				if root and hum then
					if not hitboxOriginals[p] then hitboxOriginals[p]=root.Size end
